...
Gp2=Gp1;
    ...
    disp(['the ', num2str(i), 'th step oil production would be: ' ,num2str(Np) , ' MMSTB']);
    disp(['the ', num2str(i), 'th step gas production would be: ' ,num2str(Gp) , ' MMSTB']);
    np(i)=Np;
    gp(i)=Gp;
    SNp=sum(np);
    SGp=sum(gp);
end
 disp(['the cumulative Oil production would be: ' , num2str(SNp), ' MMSTB']);
 disp(['the cumulative Gas production would be: ' , num2str(SGp), ' MMSCF']);
